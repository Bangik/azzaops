...
<div class="page-header">
    ...
    <div class="d-flex gap-2">
        <a href="{{ route('admin.reports.export', array_merge(request()->all(), ['format' => 'xlsx'])) }}" class="btn btn-success">
            <i class="bi bi-file-earmark-excel me-1"></i> Ekspor Excel (.xlsx)
        </a>
        <a href="{{ route('admin.reports.export', array_merge(request()->all(), ['format' => 'csv'])) }}" class="btn btn-outline-success">
...
<ul class="nav nav-tabs mb-3">
    <li class="nav-item">
        <a class="nav-link {{ $type === 'wo' ? 'active' : '' }}" href="{{ route('admin.reports.index', ['type' => 'wo', 'from' => $from, 'to' => $to]) }}">
            <i class="bi bi-clipboard-check me-1"></i> Work Order
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $type === 'customers' ? 'active' : '' }}" href="{{ route('admin.reports.index', ['type' => 'customers', 'from' => $from, 'to' => $to]) }}">
            <i class="bi bi-people me-1"></i> Customer
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $type === 'invoices' ? 'active' : '' }}" href="{{ route('admin.reports.index', ['type' => 'invoices', 'from' => $from, 'to' => $to]) }}">
            <i class="bi bi-receipt me-1"></i> Invoice
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $type === 'rab' ? 'active' : '' }}" href="{{ route('admin.reports.index', ['type' => 'rab', 'from' => $from, 'to' => $to]) }}">
            <i class="bi bi-calculator me-1"></i> RAB
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $type === 'finance' ? 'active' : '' }}" href="{{ route('admin.reports.index', ['type' => 'finance', 'from' => $from, 'to' => $to]) }}">
            <i class="bi bi-cash-stack me-1"></i> Keuangan
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $type === 'staff' ? 'active' : '' }}" href="{{ route('admin.reports.index', ['type' => 'staff', 'from' => $from, 'to' => $to]) }}">
...
<div class="filter-bar">
    <form action="{{ route('admin.reports.index') }}" method="GET" class="row g-2 align-items-end">
        ...
        <div class="col-md-3">
            ...
            <input type="text" name="from" class="form-control form-control-sm datepicker" value="{{ $from }}" required>
        </div>
        <div class="col-md-3">
            <label class="form-label small">Sampai Tanggal</label>
            <input type="text" name="to" class="form-control form-control-sm datepicker" value="{{ $to }}" required>
        </div>
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <div>
            <h5 class="mb-1">
                @switch($type)
                    @case('customers') Data Laporan Pelanggan @break
                    @case('invoices') Data Laporan Invoice @break
                    @case('rab') Data Laporan RAB @break
                    @case('finance') Data Transaksi Keuangan @break
                    @case('staff') Data Kinerja Staff @break
                    @default Data Laporan Work Order
                @endswitch
            </h5>
            ...
            @if($type === 'customers')
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        @empty
                        <tr><td colspan="8" class="text-center py-4 text-muted">Tidak ada data customer ditemukan</td></tr>
                        @endforelse
                    </tbody>
                </table>
            @elseif($type === 'invoices')
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @forelse($data as $inv)
                        <tr>
                            <td><a href="{{ route('admin.invoices.show', $inv) }}" class="fw-semibold text-decoration-none">{{ $inv->invoice_number }}</a></td>
                            <td>{{ $inv->customer->name }}</td>
                            <td>Rp {{ number_format($inv->subtotal, 0, ',', '.') }}</td>
                            <td>-Rp {{ number_format($inv->discount, 0, ',', '.') }}</td>
                            <td>Rp {{ number_format($inv->tax_amount, 0, ',', '.') }}</td>
                            <td class="text-end fw-bold text-primary">Rp {{ number_format($inv->total, 0, ',', '.') }}</td>
                            <td><x-status-badge :status="$inv->payment_status" /></td>
                            <td><x-status-badge :status="$inv->status" /></td>
                        </tr>
                        @empty
                        <tr><td colspan="8" class="text-center py-4 text-muted">Tidak ada data invoice ditemukan</td></tr>
                        @endforelse
                    </tbody>
                </table>
            @elseif($type === 'rab')
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @forelse($data as $r)
                        <tr>
                            <td><a href="{{ route('admin.rab.show', $r) }}" class="fw-semibold text-decoration-none">{{ $r->rab_number }}</a></td>
                            <td>{{ $r->title }}</td>
                        ...
                        @empty
                        <tr><td colspan="7" class="text-center py-4 text-muted">Tidak ada data RAB ditemukan</td></tr>
                        @endforelse
                    </tbody>
                </table>
            @elseif($type === 'finance')
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="badge {{ $t->type->value === 'income' ? 'bg-success' : 'bg-danger' }}">{{ $t->type->label() }}</span></td>
                            <td>{{ $t->category ? $t->category->name : ($t->type->value === 'income' ? 'Pembayaran Jasa' : 'Lainnya') }}</td>
                            <td>{{ $t->description }}</td>
                            <td class="text-end fw-semibold {{ $t->type->value === 'income' ? 'text-success' : 'text-danger' }}">
                                {{ $t->type->value === 'income' ? '+' : '-' }}Rp {{ number_format($t->amount, 0, ',', '.') }}
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="5" class="text-center py-4 text-muted">Tidak ada data transaksi keuangan ditemukan</td></tr>
                        @endforelse
                    </tbody>
                </table>
            @elseif($type === 'staff')
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        @empty
                        <tr><td colspan="7" class="text-center py-4 text-muted">Tidak ada data staff ditemukan</td></tr>
                        @endforelse
                    </tbody>
                </table>
            @else
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @forelse($data as $wo)
                        <tr>
                            <td><a href="{{ route('admin.work-orders.show', $wo) }}" class="fw-semibold text-decoration-none">{{ $wo->wo_number }}</a></td>
                            <td>
                                <div class="fw-semibold">{{ $wo->title }}</div>
                                <small class="text-muted">{{ $wo->type->label() }}</small>
                            </td>
                        ...
                        @empty
                        <tr><td colspan="9" class="text-center py-4 text-muted">Tidak ada data Work Order ditemukan</td></tr>
                        @endforelse
                    </tbody>
                </table>
            @endif
