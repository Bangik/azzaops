...
<div class="filter-bar">
    <form action="{{ route('admin.finance.index') }}" method="GET" class="row g-2 align-items-end">
        ...
        <div class="col-md-3 custom-date-group d-none">
            ...
            <input type="text" name="from" class="form-control form-control-sm datepicker" value="{{ $from }}">
        </div>
        <div class="col-md-3 custom-date-group d-none">
            <label class="form-label small">Sampai Tanggal</label>
            <input type="text" name="to" class="form-control form-control-sm datepicker" value="{{ $to }}">
        </div>
        ...
        <div class="card stat-card border-0">
            <div class="card-body d-flex align-items-center">
                ...
                <div>
                    ...
                    <div class="stat-value text-success">Rp {{ number_format($summary['total_income'], 0, ',', '.') }}</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card stat-card border-0">
            <div class="card-body d-flex align-items-center">
                ...
                <div>
                    ...
                    <div class="stat-value text-danger">Rp {{ number_format($summary['total_expense'], 0, ',', '.') }}</div>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <table class="table mb-0 align-middle">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge {{ $t->type->value === 'income' ? 'bg-success' : 'bg-danger' }}">
                                ...
                                <td>
                                    ...
                                    @if($t->invoice)
                                        <small class="text-muted">Invoice: <a href="{{ route('admin.invoices.show', $t->invoice) }}">{{ $t->invoice->invoice_number }}</a></small>
                                    @endif
                                </td>
                            ...
                            @empty
                            <tr>
                                <td colspan="4" class="text-center text-muted py-4">Belum ada transaksi di periode ini</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if($e->receipt_photo)
                                        <div class="mt-1"><a href="{{ asset($e->receipt_photo) }}" target="_blank" class="badge text-bg-light border text-decoration-none"><i class="bi bi-image me-1"></i> Struk</a></div>
                                    @endif
                                </td>
                                <td class="text-end text-danger fw-semibold">Rp {{ number_format($e->amount, 0, ',', '.') }}</td>
                                <td>
                                    <div class="d-flex gap-1">
                                        <a href="{{ route('admin.expenses.edit', $e) }}" class="btn btn-sm btn-warning btn-action"><i class="bi bi-pencil"></i></a>
                                        <form action="{{ route('admin.expenses.destroy', $e) }}" method="POST" onsubmit="return confirmDelete('Hapus catatan pengeluaran ini?')">
                                            @csrf @method('DELETE')
                            ...
                            @empty
                            <tr>
                                <td colspan="4" class="text-center text-muted py-4">Belum ada pengeluaran dicatat</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
