        ...
        <form action="{{ route('admin.expenses.update', $expense) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('work_order_id') is-invalid @enderror" id="work_order_id" name="work_order_id">
                        ...
                        @foreach($workOrders as $wo)
                            <option value="{{ $wo->id }}" {{ old('work_order_id', $expense->work_order_id) == $wo->id ? 'selected' : '' }}>
                ...
                <div class="col-md-3 mb-3">
                    <label for="amount" class="form-label">Nominal Nominal (Rp)</label>
                    <input type="number" class="form-control @error('amount') is-invalid @enderror" id="amount" name="amount" value="{{ old('amount', intval($expense->amount)) }}" required min="1">
                    @error('amount')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="text" class="form-control datepicker @error('expense_date') is-invalid @enderror" id="expense_date" name="expense_date" value="{{ old('expense_date', $expense->expense_date->format('Y-m-d')) }}" required>
                    @error('expense_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="file" class="form-control @error('receipt_photo') is-invalid @enderror" id="receipt_photo" name="receipt_photo" accept="image/*">
                    @if($expense->receipt_photo)
                        <div class="mt-2 small">
                            File saat ini: <a href="{{ asset($expense->receipt_photo) }}" target="_blank">Lihat Struk</a>
                        </div>
                    @endif
                    @error('receipt_photo')
                        <div class="invalid-feedback">{{ $message }}</div>
