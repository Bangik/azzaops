                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('work_order_id') is-invalid @enderror" id="work_order_id"
                            ...
                            @foreach ($workOrders as $wo)
                                <option value="{{ $wo->id }}"
                                    {{ old('work_order_id', $expense->work_order_id) == $wo->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('financial_account_id') is-invalid @enderror"
                            ...
                            @foreach ($financialAccounts as $account)
                                <option value="{{ $account->id }}" @selected(old('financial_account_id', $expense->financial_account_id) == $account->id)>{{ $account->name }}
                                    ({{ $account->code }})</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('pic') is-invalid @enderror" id="pic"
                            name="pic" value="{{ old('pic', $expense->pic) }}"
                            placeholder="Nama PIC atau pihak terkait">
                        @error('pic')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" class="form-control @error('amount') is-invalid @enderror" id="amount"
                            name="amount" value="{{ old('amount', intval($expense->amount)) }}" required min="1">
                        @error('amount')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        @if ($expense->receipt_photo)
                            <div class="mt-2 small">
                                File saat ini: <a href="{{ asset($expense->receipt_photo) }}" target="_blank">Lihat
                                    Struk</a>
                            </div>
                        @endif
                        @error('receipt_photo')
