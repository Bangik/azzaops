...
@section('content')
    ...
    <div class="card border-0 mb-4">
        <div class="card-body p-3">
            <form action="{{ route('admin.dashboard') }}" method="GET" id="dashboard-filter-form"
                ...
                <div class="col-md-6">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div>
                            <div class="fw-semibold small">Filter Akun Keuangan</div>
                            <div class="text-muted" style="font-size: 0.8rem;">
                                @if ($selectedAccount)
                                    Menampilkan data keuangan untuk <strong>{{ $selectedAccount->name }}</strong>
                                    ({{ $selectedAccount->code }})
                                @else
                                    Menampilkan data keuangan untuk <strong>Semua Akun</strong>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6">
                    <div class="d-flex gap-2 justify-content-md-end align-items-center">
                        <label for="financial_account_id"
                            class="form-label small mb-0 text-muted text-nowrap d-none d-sm-inline">Pilih Akun:</label>
                        <select name="financial_account_id" id="financial_account_id" class="form-select form-select-sm"
                            style="max-width: 250px;" onchange="document.getElementById('dashboard-filter-form').submit()">
                            <option value="">Semua Akun Keuangan</option>
                            @foreach ($financialAccounts as $account)
                                <option value="{{ $account->id }}" @selected($financialAccountId == $account->id)>
                                    {{ $account->name }} ({{ $account->code }})
                                </option>
                            @endforeach
                        </select>
                        @if ($financialAccountId)
                            <a href="{{ route('admin.dashboard') }}" class="btn btn-sm btn-outline-secondary text-nowrap"
                                title="Reset Filter">
                        ...
                        <div class="stat-label">
                            ...
                            @if ($selectedAccount)
                                <span class="badge bg-info bg-opacity-10 text-info ms-1"
                                    style="font-size: 0.65rem;">{{ $selectedAccount->code }}</span>
                            @endif
                        </div>
                ...
                <div class="card-header border-0 d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <p class="text-muted small mb-0">
                            Rangkuman 6 bulan terakhir
                            @if ($selectedAccount)
                                <span class="text-primary fw-semibold">({{ $selectedAccount->name }})</span>
                            @else
                        ...
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                @forelse($recentActions as $action)
                                    <tr>
                                        <td>
                                            <a href="{{ route('admin.work-orders.show', $action) }}"
                                        ...
                                        <td>{{ $action->scheduled_date ? $action->scheduled_date->format('d/m/Y') : '-' }}
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            @elseif($action->status === App\Enums\WorkOrderStatus::Reported)
                                                <a href="{{ route('admin.invoices.create', ['work_order_id' => $action->id]) }}"
