    ...
    <div class="page-header">
        ...
        <div class="d-flex gap-2">
            ...
            @if ($invoice->payment_status === App\Enums\PaymentStatus::Paid)
                <a href="{{ route('admin.invoices.receipt', $invoice) }}" class="btn btn-success">
                    <i class="bi bi-file-earmark-check me-1"></i> Download Kwitansi
                </a>
            @endif
    ...
    <div class="row g-3">
                        ...
                        <table class="table mb-0 align-middle">
                            ...
                            <tfoot class="table-light">
                                ...
                                @if ($invoice->discount_value > 0)
                                    <tr>
                                        <td colspan="4" class="text-end">Diskon
                                            ({{ $invoice->discount_type === 'percent' ? intval($invoice->discount_value) . '%' : 'Potongan Langsung' }})
                                        </td>
                                ...
                                @if ($invoice->tax_percentage > 0)
                                    <tr>
                                        <td colspan="4" class="text-end">PPN ({{ intval($invoice->tax_percentage) }}%)
                                        </td>
        ...
        <div class="col-lg-4">
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <div class="mb-2"><span class="text-muted small">Sudah dibayar</span>
                        <div class="fw-semibold">Rp {{ number_format($invoice->paid_amount, 0, ',', '.') }}</div>
                    </div>
                    <div class="mb-2"><span class="text-muted small">Metode</span>
                        <div class="fw-semibold">{{ $invoice->payment_method ?: '-' }}</div>
                    </div>
                    <div class="mb-2"><span class="text-muted small">Akun Keuangan</span>
                        <div class="fw-semibold">{{ $invoice->financialAccount ? $invoice->financialAccount->name : '-' }}
                        </div>
                    </div>
                    <div class="mb-0"><span class="text-muted small">Tanggal bayar</span>
                        <div class="fw-semibold">{{ $invoice->payment_date?->format('d/m/Y') ?: '-' }}</div>
                    </div>
                </div>
            </div>
            ...
            @if ($invoice->payment_status !== App\Enums\PaymentStatus::Paid)
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0">Catat Pembayaran</h5>
                    </div>
                    <div class="card-body">
                                ...
                                <select name="financial_account_id" class="form-select" required>
                                    ...
                                    @foreach (\App\Models\FinancialAccount::active()->get() as $account)
                                        <option value="{{ $account->id }}"
                                            {{ old('financial_account_id', $invoice->financial_account_id) == $account->id ? 'selected' : '' }}>
                                            {{ $account->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <button class="btn btn-success w-100"><i class="bi bi-check2-circle me-1"></i> Tandai
                                Dibayar</button>
                        </form>
                    </div>
                </div>
