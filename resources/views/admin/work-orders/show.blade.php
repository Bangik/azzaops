        ...
        <div class="d-flex gap-2">
            @if (
                $workOrder->type->code === 'checking' &&
                    ...
                    $workOrder->status !== App\Enums\WorkOrderStatus::Cancelled)
            ...
            @if (
                ...
                @if (auth()->user()->role->value !== 'admin')
                    <form action="{{ route('admin.work-orders.destroy', $workOrder) }}" method="POST"
                ...
                <div class="card-body">
                            ...
                            <div class="fw-semibold">
                                ...
                                @if ($workOrder->scheduled_time)
                                    &nbsp;<span class="text-muted small"><i class="bi bi-clock"></i>
                                        {{ date('H:i', strtotime($workOrder->scheduled_time)) }}</span>
                                @endif
                            </div>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <div class="fw-semibold">{{ $workOrder->creator->name }} <span
                                    class="text-muted small">({{ $workOrder->created_at->format('d/m/Y H:i') }})</span>
                            </div>
                        </div>
                        <div class="col-md-6">
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="fw-semibold">
                            ...
                            @if ($workOrder->gmaps_link)
                                <a href="{{ $workOrder->gmaps_link }}" target="_blank"
                                    class="ms-2 badge text-bg-light border text-decoration-none">
                        ...
                        @foreach ($workOrder->reports as $report)
                            <div class="border rounded p-3 mb-3">
                                ...
                                <div class="collapse mb-3" id="editReportCollapse-{{ $report->id }}">
                                    <form action="{{ route('admin.work-orders.update-report', [$workOrder, $report]) }}"
                                        method="POST" class="bg-light p-3 border rounded">
                                        @csrf
                                        @method('PUT')
                                        <div class="mb-3">
                                            <label for="findings-{{ $report->id }}"
                                                class="form-label small fw-bold">Temuan Lapangan</label>
                                            <textarea class="form-control form-control-sm" id="findings-{{ $report->id }}" name="findings" rows="3"
                                                required>{{ $report->findings }}</textarea>
                                        </div>
                                        <div class="mb-3">
                                            <label for="work_done-{{ $report->id }}"
                                                class="form-label small fw-bold">Pekerjaan yang Dilakukan</label>
                                            <textarea class="form-control form-control-sm" id="work_done-{{ $report->id }}" name="work_done" rows="3"
                                                required>{{ $report->work_done }}</textarea>
                                        </div>
                                        <div class="mb-3">
                                            <label for="recommendations-{{ $report->id }}"
                                                class="form-label small fw-bold">Rekomendasi</label>
                                        ...
                                        <div class="d-flex gap-2">
                                            ...
                                            <button type="button" class="btn btn-sm btn-secondary"
                                                ...
                                                data-bs-target="#editReportCollapse-{{ $report->id }}">Batal</button>
                                        </div>
                                    </form>
                                </div>
                                ...
                                @if ($report->photos->count())
                                    <div class="d-flex gap-3 flex-wrap mt-3">
                                        @foreach ($report->photos as $photo)
                                            <div class="border rounded p-1 d-flex flex-column align-items-center bg-white"
                                                style="width: 100px;">
                                                <a href="{{ $photo->photo_url }}" target="_blank"
                                                    class="position-relative" title="{{ $photo->photo_type->label() }}">
                                                    <img src="{{ $photo->photo_url }}"
                                                        alt="{{ $photo->caption ?? $photo->photo_type->label() }}"
                                                        style="width:88px;height:88px;object-fit:cover;border-radius:4px;">
                                                    <span class="badge bg-dark position-absolute bottom-0 start-0 m-1"
                                                        style="font-size:0.6rem;">{{ $photo->photo_type->label() }}</span>
                                                </a>
                                                <a href="{{ $photo->photo_url }}" download
                                                    class="btn btn-sm btn-outline-primary mt-1 w-100 py-0"
        ...
        <div class="col-lg-4">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <a href="{{ route('admin.customers.show', $workOrder->customer) }}"
                ...
                <div class="card-body">
                    ...
                    @if (!in_array($workOrder->status, [App\Enums\WorkOrderStatus::Completed, App\Enums\WorkOrderStatus::Cancelled]))
                        ...
                        <form action="{{ route('admin.work-orders.assign', $workOrder) }}" method="POST">
                            @csrf
                            <label
                                class="form-label small fw-semibold">{{ $workOrder->assignments->count() ? 'Re-assign / Tambah Teknisi' : 'Pilih Teknisi' }}</label>
                            <div class="mb-3" style="max-height:180px;overflow-y:auto;">
                                @forelse($technicians as $tech)
                                    <div class="form-check mb-1">
                                        <input class="form-check-input" type="checkbox" name="technician_ids[]"
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        @if ($workOrder->invoice)
                            <a href="{{ route('admin.invoices.show', $workOrder->invoice) }}"
                                class="btn btn-outline-primary btn-sm text-start">
                                <i class="bi bi-receipt me-1"></i> Invoice: {{ $workOrder->invoice->invoice_number }}
                            </a>
                            <a href="{{ route('admin.invoices.pdf', $workOrder->invoice) }}"
                                class="btn btn-outline-success btn-sm text-start">
                                <i class="bi bi-download me-1"></i> Download Invoice (PDF)
                            </a>
                            @if ($workOrder->invoice->payment_status === App\Enums\PaymentStatus::Paid)
                                <a href="{{ route('admin.invoices.receipt', $workOrder->invoice) }}"
                                    class="btn btn-outline-success btn-sm text-start">
                                    <i class="bi bi-file-earmark-check me-1"></i> Download Kwitansi (LUNAS)
                                </a>
                            @endif
                            <a href="{{ route('admin.work-orders.invoice-report-pdf', $workOrder) }}"
                                class="btn btn-outline-success btn-sm text-start">
                        ...
                        @if ($workOrder->reports->count())
                            <a href="{{ route('admin.work-orders.report-pdf', $workOrder) }}"
                                class="btn btn-outline-success btn-sm text-start">
                        ...
                        @if ($workOrder->rab)
                            <a href="{{ route('admin.rab.show', $workOrder->rab) }}"
                                class="btn btn-outline-primary btn-sm text-start">
                        ...
                        @if ($workOrder->parentWorkOrder)
                            <a href="{{ route('admin.work-orders.show', $workOrder->parentWorkOrder) }}"
                                class="btn btn-outline-info btn-sm text-start">
                                <i class="bi bi-link-45deg me-1"></i> WO Induk:
                                {{ $workOrder->parentWorkOrder->wo_number }}
                            </a>
                        @endif
                    </div>
                </div>
