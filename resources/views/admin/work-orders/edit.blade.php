                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        @if (auth()->user()->role->value === 'admin')
                            <input type="text" class="form-control"
                                value="{{ $workOrder->customer->display_name }} ({{ $workOrder->customer->phone }})"
                                readonly disabled>
                        ...
                        <select class="form-select @error('service_category_id') is-invalid @enderror"
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    {{ old('service_category_id', $workOrder->service_category_id) == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('vendor_id') is-invalid @enderror" id="vendor_id"
                            ...
                            @foreach ($vendors as $vendor)
                                <option value="{{ $vendor->id }}" @selected(old('vendor_id', $workOrder->vendor_id) == $vendor->id)>{{ $vendor->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('vendor_id')
                        ...
                        <select class="form-select @error('work_order_type_id') is-invalid @enderror"
                            ...
                            @foreach ($types as $type)
                                <option value="{{ $type->id }}"
                                    {{ old('work_order_type_id', $workOrder->work_order_type_id) == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" class="form-control datepicker @error('scheduled_date') is-invalid @enderror"
                            ...
                            value="{{ old('scheduled_date', $workOrder->scheduled_date ? $workOrder->scheduled_date->format('Y-m-d') : '') }}">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="time" class="form-control @error('scheduled_time') is-invalid @enderror"
                            ...
                            value="{{ old('scheduled_time', $workOrder->scheduled_time ? date('H:i', strtotime($workOrder->scheduled_time)) : '') }}">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" class="form-control @error('job_order') is-invalid @enderror" id="job_order"
                            name="job_order" value="{{ old('job_order', $workOrder->job_order) }}" min="0"
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                            name="title" value="{{ old('title', $workOrder->title) }}"
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea class="form-control @error('location') is-invalid @enderror" id="location" name="location" rows="2"
                            required>{{ old('location', $workOrder->location) }}</textarea>
                        @error('location')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="text" class="form-control @error('gmaps_link') is-invalid @enderror" id="gmaps_link"
                            name="gmaps_link" value="{{ old('gmaps_link', $workOrder->gmaps_link) }}"
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                            rows="3">{{ old('description', $workOrder->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                                ...
                                <tr class="item-row">
                                    ...
                                    <td>
                                        <input type="number" class="form-control form-control-sm text-end price-field"
                                            ...
                                            value="{{ intval($item['unit_price']) }}" min="0" required>
                                    </td>
